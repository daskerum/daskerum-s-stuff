...
export class OpenAIOperations {
    constructor(openai_key, model_name, history_length, filePath) {
        ...
        this.loadPrompt(); // Call the method to load the prompt on initialization
    }

    async loadPrompt() {
        try {
            const fileContent = await readFile(this.filePath, 'utf8');
            this.basePrompt = fileContent;
    ...
    async make_openai_call(text) {
        await this.loadPrompt(); // Ensure the latest prompt is loaded
